...
- **Per-display app list** — scope the switcher on a chosen monitor to just the apps with a window *on that monitor* (optionally surfacing even excluded apps that live there). Set it per display in **Settings → General → Displays**; mutually exclusive with mirroring onto all displays.
