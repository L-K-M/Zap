...
**Latest release:** v<!-- version -->2.0.0<!-- /version --> · [Download](https://github.com/L-K-M/Zap/releases/latest)
...
- **Per-display app list** — scope the switcher on a chosen monitor to just the apps with a window *on that monitor* (optionally surfacing even excluded apps that live there). Apps running full-screen on the monitor are included too (a full-screen app sits on its own Space, which would otherwise hide it) — togglable. Set it per display under **Settings → Displays** — a tab that appears only when you have more than one display, since scoping (and panel mirroring) only apply with multiple monitors. Mutually exclusive with mirroring onto all displays.
